...
# --- UI SETUP ---
st.set_page_config(page_title="Parental Genetics & Child Health Risk Predictor", layout="wide")
...
with col5:
    ...
    # PERBAIKAN: Mengubah label teks dari "Golongan Darah Ayah" menjadi "Golongan Darah Anak"
...
if st.button("Predict Health Risk", type="primary", use_container_width=True):
    # PENTING: Pastikan urutan array ini sama persis dengan urutan FEATURE_NAMES pada inference.py
    features = [
        father_age,
        mother_age,
        father_height,
    ...
    try:
        ...
        # Menampilkan Hasil Klasifikasi Risiko Utama
        label = result["labels"][0]
        
        st.subheader("🎯 Hasil Analisis Risiko")
        if label.lower() == "high":
            st.error(f"### Predicted Health Risk: **{label}** ⚠️ (Butuh Perhatian Khusus)")
        elif label.lower() == "moderate":
            st.warning(f"### Predicted Health Risk: **{label}** 🟡 (Risiko Sedang)")
        else:
            st.success(f"### Predicted Health Risk: **{label}** ✅ (Risiko Rendah / Aman)")
        
        # PERBAIKAN: Visualisasi diagram probabilitas yang rapi menggunakan DataFrame pandas
        if "probabilities" in result:
            ...
            if len(probs) == len(classes):
                ...
                st.bar_chart(data=chart_df, x="Risk Level", y="Probability")
            else:
                st.bar_chart(probs)